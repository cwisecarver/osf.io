...
class TestContributorDetail:
    ...
    def url_public(self, user, project_public):
        return '/{}nodes/{}/contributors/{}/'.format(API_BASE, project_public._id, user._id)
    ...
    def test_get_contributor_detail_valid_response(self, app, user, project_public, project_private, url_public, url_private):
    ...
    def test_get_contributor_detail_errors(self, app, user, url_private_base, url_private):
    ...
    #   test_get_private_node_non_contributor_detail_contributor_auth
        res = app.get(url_private_base.format(non_contrib._id), auth=user.auth, expect_errors=True)
        assert res.status_code == 404

    #   test_get_private_node_invalid_user_detail_contributor_auth
        res = app.get(url_private_base.format('invalid'), auth=user.auth, expect_errors=True)
        assert res.status_code == 404

    def test_unregistered_contributor_detail_show_up_as_name_associated_with_project(self, app, user):
        project = ProjectFactory(creator=user, is_public=True)
        project.add_unregistered_contributor('Rheisen Dennis', 'reason@gmail.com', auth=Auth(user), save=True)
        ...
        project_two.add_unregistered_contributor('Nesiehr Sinned', 'reason@gmail.com', auth=Auth(user), save=True)
    ...
    def test_detail_includes_index(self, app, user, project_public, url_public):
        ...
        other_contributor_detail = '/{}nodes/{}/contributors/{}/'.format(API_BASE, project_public._id, other_contributor._id)
...
class TestNodeContributorOrdering:
    ...
    def test_initial_order(self, app, user, contribs, project, contrib_user_id):
    ...
    def test_move_top_contributor_down_to_bottom(self, app, user, contribs, project, contrib_user_id, last_position, url_contrib_base):
        contributor_to_move = contribs[0]._id
        contributor_id = '{}-{}'.format(project._id, contributor_to_move)
        former_second_contributor = contribs[1]
        url = '{}{}/'.format(url_contrib_base, contributor_to_move)
    ...
    def test_move_bottom_contributor_up_to_top(self, app, user, contribs, project, contrib_user_id, last_position, url_contrib_base):
    ...
    def test_move_second_to_last_contributor_down_past_bottom(self, app, user, contribs, project, contrib_user_id, last_position, url_contrib_base):
    ...
    def test_move_top_contributor_down_to_second_to_last_position_with_negative_numbers(self, app, user, contribs, project, contrib_user_id, last_position, url_contrib_base):
        contributor_to_move = contribs[0]._id
        contributor_id = '{}-{}'.format(project._id, contributor_to_move)
        former_second_contributor = contribs[1]
        url = '{}{}/'.format(url_contrib_base, contributor_to_move)
        ...
        assert res.status_code == 200
        contributor_list = res.json['data']
        assert contrib_user_id(contributor_list[last_position - 1]) == contributor_to_move
        assert contrib_user_id(contributor_list[0]) == former_second_contributor._id

    def test_write_contributor_fails_to_move_top_contributor_down_one(self, app, user, contribs, project, contrib_user_id, url_contrib_base):
        contributor_to_move = contribs[0]._id
        contributor_id = '{}-{}'.format(project._id, contributor_to_move)
        former_second_contributor = contribs[1]
        url = '{}{}/'.format(url_contrib_base, contributor_to_move)
        ...
        res_patch = app.patch_json_api(url, data, auth=former_second_contributor.auth, expect_errors=True)
    ...
    def test_non_authenticated_fails_to_move_top_contributor_down_one(self, app, user, contribs, project, contrib_user_id, url_contrib_base):
        contributor_to_move = contribs[0]._id
        contributor_id = '{}-{}'.format(project._id, contributor_to_move)
        former_second_contributor = contribs[1]
        url = '{}{}/'.format(url_contrib_base, contributor_to_move)
...
class TestNodeContributorUpdate:
    ...
    def project(self, user, contrib):
        project = ProjectFactory(creator=user)
        project.add_contributor(contrib, permissions=[permissions.READ, permissions.WRITE], visible=True, save=True)
        return project
    ...
    def test_node_update_invalid_data(self, app, user, url_creator):
        res = app.put_json_api(url_creator, "Incorrect data", auth=user.auth, expect_errors=True)
        assert res.status_code == 400
...
class TestNodeContributorPartialUpdate:
    ...
    def url_contrib(self, contrib, project):
        return '/{}nodes/{}/contributors/{}/'.format(API_BASE, self.project._id, self.user_two._id)
    ...
    def test_patch_permission_only(self, app, user, project):
        ...
        project.add_contributor(user_read_contrib, permissions=[permissions.READ, permissions.WRITE], visible=False, save=True)
        url_read_contrib = '/{}nodes/{}/contributors/{}/'.format(API_BASE, project._id, user_read_contrib._id)
...
class TestNodeContributorDelete:
    ...
    def url_user_write_contrib(self, project, user_write_contrib):
        return '/{}nodes/{}/contributors/{}/'.format(API_BASE, project._id, user_write_contrib._id)

    @pytest.fixture()
    def url_user_non_contrib(self, project, user_non_contrib):
        return '/{}nodes/{}/contributors/{}/'.format(API_BASE, project._id, user_non_contrib._id)

    def test_remove_errors(self, app, user, user_write_contrib, user_non_contrib, project, url_user, url_user_write_contrib, url_user_non_contrib):

    #   test_remove_contributor_non_contributor
        res = app.delete(url_user_write_contrib, auth=user_non_contrib.auth, expect_errors=True)
        assert res.status_code == 403
    ...
    #   test_remove_non_existing_user_admin
        url_user_fake = '/{}nodes/{}/contributors/{}/'.format(API_BASE, project._id, 'fake')
    ...
    def test_remove_contributor_non_admin_is_forbidden(self, app, user_write_contrib, user_non_contrib, project, url_user_non_contrib):
        project.add_contributor(user_non_contrib, permissions=[permissions.READ, permissions.WRITE], visible=True, save=True)

        res = app.delete(url_user_non_contrib, auth=user_write_contrib.auth, expect_errors=True)
        assert res.status_code == 403
