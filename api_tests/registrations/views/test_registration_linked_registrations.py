...
class TestRegistrationLinkedRegistrationsList(LinkedRegistrationsTestCase):
    ...
    def test_read_only_contributor_can_view_private_registration_linked_registrations(self):
        res = self.make_request(
            registration_id=self.private_registration._id,
    ...
    def test_non_contributor_cannot_view_private_registration_linked_registrations(self):
        res = self.make_request(
            registration_id=self.private_registration._id,
    ...
    def test_unauthenticated_cannot_view_private_registration_linked_registrations(self):
        res = self.make_request(
            registration_id=self.private_registration._id,
            expect_errors=True
        )
...
class TestRegistrationsLinkedRegistrationsRelationship(LinkedRegistrationsTestCase):
    ...
    def test_public_registration_unauthenticated_user_can_view_linked_registrations_relationship(self):
        res = self.make_request(registration_id=self.public_registration._id)
        assert_equal(res.status_code, 200)
        linked_registration_ids = [r['id'] for r in res.json['data']]
        assert_in(self.public_linked_registration._id, linked_registration_ids)
    ...
    def test_private_registration_admin_contributor_can_view_linked_registrations_relationship(self):
        res = self.make_request(
            registration_id=self.private_registration._id,
            auth=self.admin_contributor.auth
        )
    ...
    def test_private_registration_rw_contributor_can_view_linked_registrations_relationship(self):
        res = self.make_request(
            registration_id=self.private_registration._id,
            auth=self.rw_contributor.auth
        )
    ...
    def test_private_registration_read_contributor_can_view_linked_registrations_relationship(self):
        res = self.make_request(
            registration_id=self.private_registration._id,
            auth=self.read_contributor.auth
        )
    ...
    def test_private_registration_non_contributor_cannot_view_linked_registrations_relationship(self):
        res = self.make_request(
            registration_id=self.private_registration._id,
            auth=self.non_contributor.auth,
            expect_errors=True
    ...
    def test_private_registration_unauthenticated_user_cannot_view_linked_registrations_relationship(self):
        res = self.make_request(
            registration_id=self.private_registration._id,
            expect_errors=True
        )
    ...
    def test_cannot_create_linked_registrations_relationship(self):
        res = self.app.post_json_api(self.public_url, {}, auth=self.admin_contributor.auth, expect_errors=True)
        assert_equal(res.status_code, 405)

    def test_cannot_update_linked_registrations_relationship(self):
        res = self.app.put_json_api(self.public_url, {}, auth=self.admin_contributor.auth, expect_errors=True)
        assert_equal(res.status_code, 405)

    def test_cannot_delete_linked_registrations_relationship(self):
        res = self.app.delete_json_api(self.public_url, {}, auth=self.admin_contributor.auth, expect_errors=True)
        assert_equal(res.status_code, 405)
